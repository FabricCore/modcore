...
modcore.fs.unzip = (from, target) => {
    let stream = new Packages.java.io.FileInputStream(
        paths.config.resolve(from),
    );
    modcore.io.unzip(stream, paths.config.resolve(target));
};

modcore.fs.copy = (from, target) => {
    Packages.java.nio.file.Files.copy(
        paths.config.resolve(from),
        paths.config.resolve(target),
    );
};

modcore.fs.move = (from, target) => {
